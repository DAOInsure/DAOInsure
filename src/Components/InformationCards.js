import { VStack, Text, Spacer, HStack, Progress, Image, Avatar, Tag, Skeleton, Spinner, SkeletonCircle } from "@chakra-ui/react";
...
function InformationCards({ author, loadingClaim, dateOfIncident, startTime }) {
            ...
            <Card cardTitle="Information">
                <HStack width="100%">
                    ...
                    <Spacer />
                    {
                        ...
                        :
                        <Text>{`${author.substr(0,7)}...${author.substr(-7)}`}</Text>
                    }
                </HStack>
                <HStack width="100%">
                    ...
                    <Spacer />
                    {
                        ...
                        :
                        <HStack>
                            <Text>0x234j23n2kj34</Text><FaExternalLinkAlt size="10px" />
                        </HStack>
                    }
                </HStack>
                <HStack width="100%">
            ...
            <Card cardTitle="OpenWeather Analysis">
                {
                    ...
                    <>
                    {
                        openWeatherStats.map((stat) => {
                            return (
                                <HStack width="100%">
                                    <Text>{new Date(parseInt(stat.dt) * 1000).toTimeString().substr(0,5)}</Text>
                                    <Text>{stat.weather[0].description[0].toUpperCase() + stat.weather[0].description.substr(1)}</Text>
                                    <Spacer />
                                    <Image src={`http://openweathermap.org/img/wn/${stat.weather[0].icon}.png`} />
                                </HStack>
                            )     
                        })
                    }
