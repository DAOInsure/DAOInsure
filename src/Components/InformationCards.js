    ...
    <VStack spacing={5}>
      <Card cardTitle="Information">
          ...
          {loadingClaim ? (
            ...
            <HStack>
              <a href={`https://ipfs.io/ipfs/` + ipfsHash} target="_blank">
                <Text>
                  {" "}
                  {`${ipfsHash.substr(0, 7)}...${ipfsHash.substr(-7)}`}{" "}
                </Text>
        ...
        <HStack width="100%">
          ...
          <Spacer />
          {loadingClaim ? (
            <Skeleton isLoaded={!loadingClaim}>Start Date</Skeleton>
          ) : (
        ...
        <HStack width="100%">
          ...
          <Spacer />
          {loadingClaim ? (
            <Skeleton isLoaded={!loadingClaim}>End Date</Skeleton>
          ) : (
            <Text>{new Date(Date.now()).toDateString()}</Text>
          )}
        </HStack>
      ...
      <Card cardTitle="Time">
        <VStack width="100%">
          <HStack width="100%">
            ...
            <Spacer />
            {loadingClaim ? (
              <Skeleton isLoaded={!loadingClaim}>Timezone</Skeleton>
            ) : (
          ...
          <HStack width="100%">
            ...
            <Spacer />
            {loadingClaim ? (
              <Skeleton isLoaded={!loadingClaim}>Date Of Incident</Skeleton>
            ) : (
              <HStack>
      ...
      <Card cardTitle="Current Results">
        <VStack width="100%">
          ...
          <Progress
            ...
            background="gray.300"
            height="10px"
            value={loadingClaim ? 0 : (yesVotes / (yesVotes + noVotes)) * 100}
            colorScheme="green"
            size="lg"
          />
          <HStack width="100%">
                ...
                <HStack width="100%">
                  ...
                  <Text>
                    {stat.weather[0].description[0].toUpperCase() +
                      stat.weather[0].description.substr(1)}
                  </Text>
                  <Spacer />
                  <Image
